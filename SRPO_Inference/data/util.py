...
def augment(img_list, hflip=True, rot=True, color=False):
    """horizontal flip OR rotate (0, 90, 180, 270 degrees)"""
    hflip = hflip and random.random() < 0.5
    vflip = rot and random.random() < 0.5
    rot90 = rot and random.random() < 0.5
    mixcolor = color and random.random() < 0.5
    ...
    def _augment(img):
        ...
        if mixcolor:
            alfa = (random.random()+1)/2
            back_color = random.random()
            img = img * alfa + back_color * (1-alfa)
        return img
